...
ContinuousScaleType = TypeAliasType(
    "ContinuousScaleType", L["identity", "linear", "log", "pow", "sqrt", "symlog", "time", "utc"]
)
...
GridInterpolate = TypeAliasType(
    "GridInterpolate", L["barycentric", "linear", "nearest", "none", "random-walk"]
)
...
LabelArrow = TypeAliasType(
    "LabelArrow", (L["auto", "down", "left", "none", "right", "up", False, True] | None)
)
...
ScaleName = TypeAliasType(
    "ScaleName", L["color", "fx", "fy", "length", "opacity", "r", "symbol", "x", "y"]
)
...
SelectFilter = TypeAliasType(
    "SelectFilter",
    L["first", "last", "maxX", "maxY", "minX", "minY", "nearest", "nearestX", "nearestY"],
)
...
StackOrderName = TypeAliasType(
    "StackOrderName", L["appearance", "inside-out", "sum", "value", "x", "y", "z"]
)
...
StackOrder = TypeAliasType(
    "StackOrder",
    L["-appearance", "-inside-out", "-sum", "-value", "-x", "-y", "-z"]
    | Sequence[Any]
    | StackOrderName
    | str,
)
