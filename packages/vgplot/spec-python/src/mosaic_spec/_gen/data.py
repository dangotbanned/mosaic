...
- `"spatial"`: Load a spatial data file format via `ST_Read`.'''
    ...
    where: Sequence[str] | str
    '''A filter (WHERE clause) to apply upon load. Only rows that pass the filter are included.'''
DataDefinition = TypeAliasType('DataDefinition', DataArray | DataCSV | DataFile | DataJSON | DataJSONObjects | DataParquet | DataQuery | DataSpatial | DataTable)
Data = TypeAliasType('Data', Mapping[str, DataDefinition])
'''Top-level dataset definitions.'''


__all__ = ("Data","DataArray","DataCSV","DataDefinition","DataFile","DataJSON","DataJSONObjects","DataParquet","DataQuery","DataSpatial","DataTable",)
